...
def vis_box(x, img, path, df, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    
    img_name = os.path.basename(path)
    # clss = ['person','car','traffic_light']
    clss = ['car', 'motorcycle', 'truck','single-unit']
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    x1,y1,x2,y2 = int(x[0]), int(x[1]), int(x[2]), int(x[3])
    df.loc[len(df)] = [img_name, x1,y1,x2,y2,clss.index(label.split(' ')[0])]
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        df = vis_box(xyxy, im0, path, df, label=label, color=colors[int(cls)], line_thickness=3)
    return df
# with torch.no_grad():
#     detect('d3.mp4')
