...
wdir = 'weights/pave' + os.sep  # weights dir
last = wdir + 'last.pt'
best = wdir + 'best.pt'
...
def train(hyp):
    ...
    if len(n):
        ...
        fresults, flast, fbest = 'results%s.txt' % n, wdir + str(epoch) +'last%s.pt' % n, wdir + str(epoch)  + 'best%s.pt' % n
        for f1, f2 in zip([wdir + str(epoch) +'last.pt', wdir + str(epoch) + 'best.pt', 'results.txt'], [flast, fbest, fresults]):
