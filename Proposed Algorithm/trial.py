...
def detect(source, save_img=True):
    ...
    weights = 'weights/pave/Vishal/yolo_vl/54jc/last.pt'    #Pass the correct path of the model
    view_img = True
    save_txt = False 
    imgsz  = 640
    device = ''
    ...
    augment=False
    iou_thres = 1
    # classes = None;conf_thres = 0.01;agnostic_nms = False
    classes = None;conf_thres = 0.17;agnostic_nms = False #Vm
...
with torch.no_grad():
    ...
    cnts = ['Japan']    #VM
    for cnt in cnts:
        img_source =os.path.join('../Trainer/test1',cnt,'images')   #Pass the correct path of the test2 dataset
        ...
        for imgcur in imgs:
            ...
            if len(annts_list)>0:
                ...
                with open('tttt3_new.txt','a+') as writer:
                    txt=str(sub_list[0])+","
